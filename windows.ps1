...
# $action = New-ScheduledTaskAction -WorkingDirectory $env:AZ_BATCH_TASK_WORKING_DIR -Execute 'Powershell.exe' -Argument \\\"Get-Counter -Counter {string.Join(",", counters)} -SampleInterval 2 -Continuous | Export-Counter -Path '{resultsFile}' -Force -FileFormat 'BLG'\\\" ; 
# $principal = New-ScheduledTaskPrincipal -UserID 'NT AUTHORITY\\SYSTEM' -LogonType ServiceAccount -RunLevel Highest ; 
# Register-ScheduledTask -Action $action -Principal $principal -TaskName '{taskName}' -Force ; 
