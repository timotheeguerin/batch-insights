...
# python .\nodestats.py *>> .\node-stats.log
$action = New-ScheduledTaskAction -WorkingDirectory $env:AZ_BATCH_TASK_WORKING_DIR -Execute 'Powershell.exe' -Argument "\$env:APP_INSIGHTS_KEY=$env:APP_INSIGHTS_KEY;Start-Process python -ArgumentList .\nodestats.py  -RedirectStandardOutput .\node-stats.log -RedirectStandardError .\node-stats.err.log -NoNewWindow"  
